...
net.core.default_qdisc=${currentQueueText}
net.ipv4.tcp_congestion_control=${currentBBRText}
net.ipv4.tcp_ecn=${currentECNValue}
EOF
    isSysctlText=$(sysctl --system 2>&1 | grep "No such file")
    echo
...
function removeOptimizingSystemConfig(){
    removeBbrSysctlConfig
    rm -f /etc/sysctl.d/99-optimize.conf
...
# 由 install_kernel.sh 生成，请勿手动修改，使用脚本卸载
fs.file-max = 1000000
...
# TCP 缓冲区（覆盖 1G~10G 全带宽场景）
# BDP = 带宽 × RTT，10G × 100ms = 125MB，设 256MB 足够所有场景
net.core.rmem_max = 268435456
net.core.wmem_max = 268435456
net.ipv4.tcp_rmem = 4096 87380 268435456
net.ipv4.tcp_wmem = 4096 16384 268435456
# UDP 缓冲区（Hysteria2/QUIC 场景）
...
EOF
    # netdev_budget_usecs 仅内核 5.2+ 支持
    local kernel_ver=$(uname -r | awk -F. '{print $1*100+$2}')
    if [ "$kernel_ver" -ge 502 ]; then
        echo "net.core.netdev_budget_usecs = 8000" >> /etc/sysctl.d/99-optimize.conf
    fi
    # conntrack 优化（tproxy/redirect 模式下关键，模块未加载则跳过）
    if sysctl net.netfilter.nf_conntrack_max >/dev/null 2>&1; then
        cat >> /etc/sysctl.d/99-optimize.conf <<-EOF
# conntrack 连接追踪优化（代理转发场景）
...
function startIpv4(){
    cat > /etc/sysctl.d/97-ipforward.conf <<-EOF
# 由 install_kernel.sh 生成
